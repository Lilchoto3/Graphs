...
while len(connections_map) < len(room_graph):
    ...
    # do a BFT disregarding backtracking until a new space is found
    # then compare that to backtracking the DFT; pick the shorter
    # option
    ...
    # TODO: implement this BFT to run before the backtracking
    # bft_queue = []
    # bft_visited = {}
    # bft_queue.append(cur_room)
    # while len(bft_queue) > 0:
    #     cur_bft_room = bft_queue.pop(0)
    #     for d in cur_bft_room.get_exits():
    #         other_bft_room = cur_bft_room.get_room_in_direction(d)
    #         # update connection map while searching
    #         if other_bft_room.id not in connections_map:
    #             connections_map[other_bft_room.id] = {d:'?' for d in other_bft_room.get_exits()}
    #         # add the connections
    #         if connections_map[cur_bft_room.id][d] == '?' or connections_map[other_bft_room.id][opp_dir(d)] == '?':
    #             connections_map[cur_bft_room.id][d] = other_bft_room.id
    #             connections_map[other_bft_room.id][opp_dir(d)] = cur_bft_room.id
    #         # add the room if it's not already been visited or in the queue
    #         # also add the whole room data, not just id
    #         if other_bft_room.id not in bft_visited and other_bft_room.id not in bft_queue:
    #             bft_queue.append(other_bft_room)
        
        # create links backward to connect the results of the bft
